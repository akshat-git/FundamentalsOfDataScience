...
## Week 9 — implementation (`Week9 - Exam 2/`)

Week 9 is organized as **Exam 2** in **`Week9 - Exam 2/`**, with a single notebook **`exam2.ipynb`** that mixes **written multiple-choice answers** (questions on probability, testing, and related course concepts) with one **simulation-based hypothesis test** in code. The implementation uses **`datascience`**, **NumPy**, and **`sample_proportions`** to draw 1,000 samples of size 300 from a four-category null distribution (`world_pop`), and defines a test statistic as the **sum of absolute differences** between expected and observed category proportions (a total-variation-style measure). It compares an **`observed_sample`** to that null model, builds a **`simulated_stats`** array under the null, and computes an **empirical p-value** as the fraction of simulated statistics at least as extreme as the observed value. The notebook records selected letter answers for each exam item (with some marked corrections in the cells). No data CSVs are bundled in this folder—the simulation uses **`make_array`** proportions defined in the notebook itself.
